...
### 가장 쉬운 길 — 더블클릭 (아무것도 몰라도)
이 저장소 폴더 맨 위에서:
- **macOS**: `SETUP_MAC.command` 더블클릭 (Homebrew 없으면 자동 설치 → 도구 설치 → 폴더 생성)
- **Windows**: `SETUP_WINDOWS.bat` 더블클릭 (winget으로 도구 설치 → 폴더 생성)

창이 뜨면 프로젝트 이름/위치만 입력하면 끝납니다. 첫 실행 경고·권한 안내는 `START_HERE.md` 참고.
...
### 방법 (OS별)
- **macOS**: Finder에서 `01_source/render.command` 더블클릭 — 또는 터미널 `cd 01_manuscript && Rscript _scripts/render_with_insertions.R`
- **Windows**: 탐색기에서 `01_source\render.bat` 더블클릭 — 또는 PowerShell `cd 01_manuscript; Rscript _scripts\render_with_insertions.R`
- **Obsidian(공통)**: 명령 팔레트에서 "Render manuscript" (Shell commands 등록 시)

> 전제: R + Quarto가 PATH에 있어야 합니다(설치 후 **새 터미널 창**에서 실행). `quarto --version`, `Rscript -e 'R.version.string'`로 확인.
...
2. 저장 — **키를 명령에 바로 붙여넣는 방식을 권장**합니다(대화형 입력이 안 되는 터미널에서도 동작):
   ```bash
   # macOS / Linux
   bash _secrets/set_zotero_key.sh "API_KEY_붙여넣기" "userID숫자"
   ```
   ```powershell
   # Windows
   powershell -ExecutionPolicy Bypass -File _secrets\set_zotero_key.ps1 -ApiKey "API_KEY" -UserId "userID"
   ```
   > **"커맨드는 실행되는데 키 입력이 안 된다"** → 위처럼 키를 인자로 전달하세요. 인자 없이 실행하면 대화형으로 묻고, 이때 입력은 일부러 **화면에 보이게** 했습니다(가리면 일부 터미널에서 입력 자체가 막힘).
...
- **렌더 오류 "cannot open file '01_source/apa.csl'" 또는 references.bib 없음** → 새 프로젝트에 apa.csl/references.bib가 없던 버그(수정됨). `bootstrap.sh`를 다시 실행하거나, `manuscript_format/apa.csl`을 `01_source/`로 복사하고 빈 `references.bib`를 만드세요.
- **citation key가 전부 공란** → Better BibTeX 미설치. Zotero 기본엔 키 개념이 없습니다. `install/zotero_bbt_setup.md` §1대로 BBT `.xpi`를 **우클릭 저장 후** Tools→Plugins에서 설치하고 **재시작**. 그래도 공란이면 BBT Preferences의 citation key format 채우고 Refresh BibTeX key.
- **Zotero API 키 입력이 안 됨(커맨드는 실행됨)** → 키를 인자로 전달: `bash _secrets/set_zotero_key.sh "KEY" "USERID"` (Windows는 `-ApiKey -UserId`). §10.
- **Windows 렌더 오류** → `01_source\render.bat` 사용. R/Quarto 설치 후 **새 창**에서 실행(PATH 갱신). HOME 관련 오류는 수정됨(맥 전용 처리).
...
- **Figure 1을 PDF로 줬는데 Windows에서 변환 실패** → Figure 1 PDF→PNG 변환은 맥 전용(`qlmanage`)입니다. Windows는 Figure 1을 직접 PNG(`03_assets/figures/Figure_1_*.png`)로 두세요.
