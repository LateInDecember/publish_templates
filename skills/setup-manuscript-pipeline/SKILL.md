...
- `install/install.sh`(mac/linux), `install/install.ps1`(windows): Quarto·R(+패키지)·Obsidian·Zotero를 **실제로 설치**(필수 도구).
- `install/bootstrap.sh`: **기본적으로 설치를 실행** → Phase 1 폴더 + Phase 4 양식·스크립트 + .obsidian 스타터 + _secrets + AGENTS.md → verify. (`--no-install`로만 설치 생략.)
...
- **R 필수** — 원고 렌더 `render_with_insertions.R`가 R 기반이라 R + 패키지 `officer, png, stringr, xml2, zip`를 설치한다. (분석 자체는 Python/MATLAB도 가능하고 `02_anal`은 도구 중립 구조지만, 렌더용 R은 반드시.)

**설치를 직접 실행한다(중요).** 단순히 "설치하세요"라고 안내만 하고 넘어가지 말 것 — 그러면 파일만 clone되고 도구는 안 깔린다. 로컬 셸 접근이 있으면(Codex CLI, Claude Code, 사용자 터미널 등) `bash references/install/install.sh`(Windows는 `install.ps1`)를 **에이전트가 실제로 실행**한다. bootstrap을 쓰면 기본적으로 이 설치가 함께 돈다.

단, 셸이 격리/원격이라 사용자 컴퓨터에 설치할 수 없는 경우(예: Cowork 샌드박스는 사용자 맥과 별개 환경)에는 추측해서 건너뛰지 말고, **사용자에게 정확한 한 줄 명령을 주어 직접 실행하게 하고 완료를 확인**한 뒤 다음 Phase로 간다. (GUI 앱 설치 후 플러그인 승인·BBT export 지정은 어차피 사용자가 앱에서 직접 한다.)
