...
bash install/bootstrap.sh "<새 프로젝트 절대경로>"               # 기본: Quarto·R·Obsidian·Zotero 자동 설치 + 폴더·양식 생성
bash install/bootstrap.sh "<새 프로젝트 절대경로>" --no-install   # 설치 건너뛰고 폴더·양식만
# Windows: powershell -ExecutionPolicy Bypass -File install/install.ps1   (그 뒤 폴더 생성은 에이전트/수동)
```

bootstrap은 **기본적으로 도구를 직접 설치**합니다(단순 안내가 아님). **Quarto·R은 필수**이고, 생성되는 `02_anal`은 도구 중립 구조입니다(R/Python 이름 없음, `01_data/{00_raw,01_interim,02_final}`). 분석 자체는 Python/MATLAB로 해도 되지만 렌더용 R은 설치됩니다.

> ⚠️ 격리 셸(예: Cowork 샌드박스)에서는 사용자 컴퓨터에 설치할 수 없습니다. 사용자 맥의 터미널, 또는 로컬 셸을 가진 에이전트(Codex CLI/Claude Code)에서 실행해야 실제 설치가 일어납니다.
...
- **마커 기반 삽입**: `{{table:1}}`, `{{figure:1}}` 마커를 렌더 시 실제 파일로 치환.
